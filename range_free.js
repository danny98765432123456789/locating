...
function timeout_insert_lower(anchors, max_nochange_time, rssi_decrease) {
  for (var i = 0; i < anchors.length; i++) {
    if (Math.floor(Date.now() / 1000) > (anchors[i].timer + max_nochange_time)) {
...
function calculation(anchors, fresh_table) {
  ...
  // tag_x = x_up / x_down;
  // tag_y = y_up / y_down;
  for (var q = 0; q < fresh_table.length; q++) {
    // console.log(fresh_table);
    // console.log(anchors);
    // console.log(anchors[fresh_table[q]-1]);
    x_up += anchors[fresh_table[q]-1].x_label;
    y_up += anchors[fresh_table[q]-1].y_label;
  }
  tag_x = x_up/fresh_table.length;
  tag_y = y_up/fresh_table.length;
    // console.log(tag_x);
